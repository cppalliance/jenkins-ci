...
Another idea: it was suggested that a GitHub Pages implementation could store all doc previews from all target repos within one centrally managed repository. This would solve permissions issues on the separate distributed repos. Google: github recommended maximum size of repository . And the search result shows: "5 GB While GitHub recommends keeping repositories under 5 GB, there are defined storage limits based on your plan: GitHub Free and Pro accounts have a 2 GB limit per repository, GitHub Team has a 4 GB limit, and GitHub Enterprise Cloud has a 5 GB limit." As of early 2026 the S3 bucket hosting previews contains 50GB. Planning for the future that means 100GB. If the doc previews were more widely rolled out to all boostorg repos, then 100GB becomes 300GB if you plan for similar storage consumption and increased usage over time. While 300GB is nothing on S3, it's 60 times larger than the recommended max repo size on GitHub. That is clearly a significant problem. 
